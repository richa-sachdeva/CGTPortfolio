...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result = m_GroundModel->Initialize(m_D3D->GetDevice(), L"../Engine/data/Checkered_Floor.dds", "../Engine/data/ground.txt");
	if(!result)
	{
	...
	for (int i=0; i<3; i++)
	{
		...
		result = m_WallModel[i]->Initialize(m_D3D->GetDevice(), L"../Engine/data/Checkered_Floor.dds", "../Engine/data/wall.txt");
		if(!result)
		{
	...
	result = m_SphereModel->Initialize(m_D3D->GetDevice(), L"../Engine/data/Checkered_Floor.dds", "../Engine/data/sphere.txt");
	if(!result)
	{
	...
	result = m_ParticleSystem->Initialize(m_D3D->GetDevice(), L"../Engine/data/snow.dds");
	if(!result)
	{
		return false;
	}
	...
	result = m_Bitmap->Initialize(m_D3D->GetDevice(), screenWidth, screenHeight, L"../Engine/data/startScreen.png", screenWidth, screenHeight);
	if(!result)
	{
