...
bool SoundClass::Initialize(HWND hwnd)
{
	...
	result = LoadWaveFile("../Engine/data/windchimes.wav", &m_music);
	if(!result)
	{
		return false;
	}
...
bool SoundClass::InitializeDirectSound(HWND hwnd)
{
	...
	waveFormat.nSamplesPerSec = 11025; // 44100 for 1411 kbps, for 88kbps its 2750
...
bool SoundClass::LoadWaveFile(char* filename, IDirectSoundBuffer8** secondaryBuffer)
{
	...
	if(waveFileHeader.sampleRate != 11025)
	{
		return false;
	}
...
bool SoundClass::Frame(float frameTime)
{
	...
	if(m_globalMute){
		result = m_music->SetVolume(DSBVOLUME_MIN);
		if(FAILED(result))
		{
			return false;
		}
		...
		result = m_music->SetVolume(DSBVOLUME_MAX/2);
		if(FAILED(result))
		{
			return false;
		}
