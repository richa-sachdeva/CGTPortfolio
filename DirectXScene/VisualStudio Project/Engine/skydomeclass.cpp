...
bool SkyDomeClass::Initialize(ID3D11Device* device)
{
	...
	result = LoadTexture(device, L"../Engine/data/checksskydome.dds");
	if(!result)
	{
		return false;
	}
...
bool SkyDomeClass::LoadTexture(ID3D11Device* device, WCHAR* filename)
{
	bool result;
...
void SkyDomeClass::ReleaseTexture()
{
