...
bool TranslateShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"shaders/translate.vs", L"shaders/translate.ps");
	if(!result)
	{
		return false;
	}
...
bool TranslateShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "TranslateVertexShader", "vs_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "TranslatePixelShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 
