...
bool TextClass::Render(ID3D11DeviceContext* deviceContext, FontShaderClass* FontShader, D3DXMATRIX worldMatrix, D3DXMATRIX orthoMatrix)
{
	...
	result = UpdateSentence(m_sentence12, " ", 10, 320, 1.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
...
bool TextClass::SetRenderCount(int count, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence11, renderString, 10, 130, 0.0f, 1.0f, 0.0f, deviceContext);	// was 290
	if(!result)
	{
		return false;
	}
...
bool TextClass::SetIntersection(bool intersection, ID3D11DeviceContext* deviceContext)
{
	...
	//if(intersection){
	//	strcat_s(tempString, "Yes");
	//}
	//else{
	//	strcat_s(tempString, "No");
	//}
	//// Setup the cpu string.
	//strcpy_s(renderString, "Intersection: ");
	//strcpy_s(renderString, tempString);

	//// Update the sentence vertex buffer with the new string information.
	//result = UpdateSentence(m_sentence12, renderString, 10, 320, 0.0f, 1.0f, 0.0f, deviceContext);
	//if(!result)
	//{
	//	return false;
	//}
	...
	if(intersection)
	{
		...
		result = UpdateSentence(m_sentence12, intersectionString, 10, 150, 0.0f, 1.0f, 0.0f, deviceContext); // was 320
	}
	else
	{
		strcpy_s(intersectionString, "Intersection: No");
		result = UpdateSentence(m_sentence12, intersectionString, 10, 150, 1.0f, 0.0f, 0.0f, deviceContext);
	}

	return result;
}

bool TextClass::SetCollision(int count, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char renderString[32];
	bool result;
	...
	// Setup the cpu string.
	strcpy_s(renderString, "Collision: ");
	strcat_s(renderString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence13, renderString, 10, 170, 0.0f, 1.0f, 0.0f, deviceContext); // was 350
	if(!result)
	{
		return false;
	}
...
bool TextClass::SetPointsCollected(int count, ID3D11DeviceContext* deviceContext){
	char tempString[16];
	char renderString[32];
	bool result;
	...
	// Setup the cpu string.
	strcpy_s(renderString, "Score: ");
	strcat_s(renderString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence14, renderString, 10, 190, 0.0f, 1.0f, 0.0f, deviceContext); // was 380
	if(!result)
	{
		return false;
	}
